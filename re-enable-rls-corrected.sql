-- =====================================================
-- RE-ENABLE RLS WITH PROPER POLICIES - CORRECTED
-- Only for tables that actually exist with proper columns
-- =====================================================
...
-- Step 4: Only create policies for exam tables if they exist and have user_id columns
-- Check if attempts table exists and has user_id column
DO $$ 
BEGIN
    IF EXISTS (
...
-- Check if attempt_items table exists and has user_id column
DO $$ 
BEGIN
    IF EXISTS (
        SELECT 1 FROM information_schema.columns 
        WHERE table_name = 'attempt_items' AND column_name = 'user_id'
    ) THEN
...
-- Check if exam_results table exists and has user_id column
DO $$ 
BEGIN
    IF EXISTS (
        SELECT 1 FROM information_schema.columns 
...
SELECT 'Only tables with user_id columns got user-specific policies.' AS note;
